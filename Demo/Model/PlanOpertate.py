from TestModel.dbModels import VersionPlan, StagePlan, Staff, AssignPlan
...
class PlanOperate:
    ...
    def __init__(self, plan_id=''):
        if plan_id:
            version_plan = VersionPlan.objects.filter(sys_name=plan_id, status=True).order_by('-update_time')
            if version_plan:
